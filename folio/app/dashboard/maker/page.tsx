...
function statusFor(l: ListingRow): { label: string; className: string } {
  if (l.status === "pending" && l.is_published) {
    return { label: "Live · edit in review", className: "text-accent-gold border-accent-gold/30 bg-accent-gold/[0.08]" };
  }
  if (l.status === "pending") {
    return { label: "In review", className: "text-accent-gold border-accent-gold/30 bg-accent-gold/[0.08]" };
  }
  if (l.status === "rejected") {
    return { label: "Not approved", className: "text-red-400 border-red-400/30 bg-red-400/[0.08]" };
  }
  if (!l.is_published) {
    return { label: "Paused", className: "text-ink-muted border-white/[0.15] bg-white/[0.04]" };
  }
  return { label: "Live", className: "text-accent-green border-accent-green/30 bg-accent-green/[0.08]" };
}

// The Maker tab: the visitor's own submissions and their status.
export default async function MakerDashboardPage({
  ...
  const hasPublished = listings.some((l) => l.is_published);

  return (
    <DashboardShell user={user} profile={profile} isAdmin={isAdmin} hasPublishedListing={hasPublished}>
      {submitted && (
      ...
      {deleted && (
        <div className="rounded-xl border border-white/[0.1] bg-white/[0.04] p-4 mb-6 text-sm text-ink-secondary">
          Listing deleted.
        </div>
      )}
      ...
      {listings.length === 0 ? (
        ...
        <div className="space-y-4">
          <p className="text-xs text-ink-muted">
            Edits to a live listing go back through review, and the current version stays live in
            the meantime.
          </p>
          {listings.map((l) => {
